...
const chat = module.exports = {
    ...
    validate: function(message) {
        let messageFixed = {group_id: db.ObjectId(message.group_id), sender_id: db.ObjectId(message.sender_id), content: message.content}
        if(messageFixed.group_id && messageFixed.sender_id) {
            return messageFixed
        } else {
            return null
        }
    ...
    sendData: function(res, _id) {
        db.chats.aggregate([{
           ...
           $unwind: '$persons' 
            ...
            $addFields: {
                senderFirstName: '$persons.firstName',
                senderLastName: '$persons.lastName'
            } 
        } ,{    
            $match: { group_id: _id}       
        },{
